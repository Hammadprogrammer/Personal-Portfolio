...
export default function Navbar() {
  ...
  // Effect to handle click outside the mobile menu
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        isOpen &&
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          width: typeof window !== "undefined" && window.innerWidth <= 576 ? "auto" : "auto",
        }}
>
          {/* Logo */}
          <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
          ...
          <div className="flex items-center space-x-4">
            ...
            {/* Mobile Menu Button - We simplify this to always show the Menu icon as the close icon is now inside the drawer */}
            <motion.button
              whileHover={{ scale: 1.1 }}
              whileTap={{ scale: 0.9 }}
              onClick={() => setIsOpen(!isOpen)}
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ref={menuRef}
            // NEW ANIMATION: Slide in from the left
            initial={{ x: '-100%', opacity: 0 }}
            animate={{ x: 0, opacity: 1 }}
            exit={{ x: '-100%', opacity: 0 }}
            transition={{ type: 'tween', duration: 0.3 }}
            className="md:hidden fixed inset-y-0 left-0 w-64 bg-background/95 backdrop-blur-xl border-r border-border/50 shadow-2xl z-30" // Fixed position and width
          >
            <div className="px-4 py-6 space-y-3">
              ...
              {navItems.map((item, index) => (
                <motion.div
                  ...
                  initial={{ opacity: 0, x: -20 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ delay: index * 0.1 + 0.1 }} // Added slight delay after drawer opens
                >
                  <Link
                    href={item.href}
                    onClick={() => setIsOpen(false)}
